/*
 * NML Edge Worker — main entry point and event loop.
 *
 * Usage:
 *   ./edge_worker [--name NAME] [--port PORT] [--data FILE] [--require-signed]
 *
 * The worker:
 *   1. Joins UDP multicast (239.78.77.76:7776)
 *   2. Sends MSG_ANNOUNCE so Python agents discover it
 *   3. Receives programs via MSG_PROGRAM, optionally verifies signature,
 *      executes in-process, and reports the score via MSG_RESULT
 *   4. Sends MSG_HEARTBEAT every HEARTBEAT_INTERVAL seconds
 */
...
/* Node identity — populated once in main() before the first ANNOUNCE */
                            ...
                            int require_signed)
{
    ...
    /* Compute program hash for result reporting */
    char phash[17];
    crypto_program_hash(body, phash);

    printf("[%s] executing program %s (signed=%d) from %s\n",
    ...
    if (rc == 0) {
        ...
        fprintf(stderr, "[%s] no score key in memory dump (hash=%s)\n",
                agent_name, phash);
    } else {
...
int main(int argc, char **argv)
{
    ...
    /* Derive node identity before first ANNOUNCE */
    ...
    while (g_running) {
        ...
        if (received > 0) {
            ...
            /* Self-filter: discard our own multicast reflections */
            ...
            case MSG_HEARTBEAT:
                /* silently track — could update a peer table here */
                break;
